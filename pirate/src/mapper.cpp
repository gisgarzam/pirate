  ...
  switch (number) {
	  ...
	  case 37: // Front module (21) writes to control0Mode[1] in Arduino - Motor 0
			...
			case 58: // [TRACK left]
							if (value==127)	 {
								if(inAuto) {
									// Specify Goal in this case clamp up in 90 degrees (vertical) for a 110mm pipe
									goal.name=pirate::PirateGoal::CLAMP_STRAIGHT;
									goal.pipeDiameter=110;
									goal.distance=0;
									// Send goal
			...
			case 59: // [TRACK right]
							if (value==127)	 {
								if(inAuto) {
									// Specify Goal in this case clamp up in 90 degrees (vertical)  for a 90mm pipe
									goal.name=pirate::PirateGoal::CLAMP_SIDEWAYS;
									goal.pipeDiameter=90;
									goal.distance=0;
									// Send goal
									ac.sendGoal(goal, boost::bind(&Mapper::doneCb, this, _1, _2), Client::SimpleActiveCallback(), Client::SimpleFeedbackCallback());
								}
